...
#import "rectVC.h"
#import "rectTableViewCell.h"

@interface rectVC ()<UITableViewDataSource,UITableViewDelegate>

/** 导航栏右侧item */
@property (nonatomic ,strong)UIButton *appointmentBtn;
/** 自定义tabbleView */
@property (nonatomic ,strong)UITableView *rectabBleView;

@end

@implementation rectVC
...
{
    _rectabBleView = [[UITableView alloc]initWithFrame:CGRectMake(0,0, kSCREENW, kSCREENH) style:(UITableViewStylePlain)];
    _rectabBleView.showsVerticalScrollIndicator = NO;
    _rectabBleView.delegate = self;
    _rectabBleView.dataSource = self;
    _rectabBleView.separatorStyle = UITableViewCellSeparatorStyleNone;
    [self.view addSubview:_rectabBleView];
...
{
    static NSString *ID = @"rectCell";
    
    rectTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    
    if (!cell)
    {
        cell = [[[NSBundle mainBundle]loadNibNamed:@"rectTableViewCell" owner:self options:nil]lastObject];
    }
